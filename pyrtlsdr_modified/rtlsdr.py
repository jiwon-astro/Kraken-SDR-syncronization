...
class BaseRtlSdr(object):
    ...
    # Added to package file on 250415
    # for GPIO control
    def set_bias_tee_gpio(self, gpio_pin, enabled):
        result = librtlsdr.rtlsdr_set_bias_tee_gpio(self.dev_p, gpio_pin,int(enabled))
        if result < 0:
            raise LibUSBError(result, f'Could not set Bias Tee at GPIO {gpio_pin}')
        return result
...
class RtlSdr(BaseRtlSdr):
    ...
    def cancel_read_async(self):
        ...
        if result < 0 and not self.read_async_canceling:
            self.close() # if it isn't canceled, the connection will closed.
